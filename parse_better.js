...
for (var i = id.length - 1; i >= 0; i--) {
  ...
  results.push({
    link : "http://www.cyscc.gov.tw/chinese/Discussing_Detail.aspx?s=" + id[i],
    id : id[i],
    county : "嘉義縣",
    bill_no : $('#ctl00_ContentPlaceHolder1_fvDetail_Label7').text(),
    type : $('#ctl00_ContentPlaceHolder1_fvDetail_Label2').text(),
 	category : $('#ctl00_ContentPlaceHolder1_fvDetail_lbType').text().replace(/\./g, "、"),
 	proposed_by : $('#ctl00_ContentPlaceHolder1_fvDetail_Label8').text().replace(/\r/g,"").replace(/縣長 {0,5}張花冠/g,"張花冠").replace(/\./g, "、").split('、'),
	petitioned_by :  $('#ctl00_ContentPlaceHolder1_fvDetail_lbParliamentary').text().replace(/\r/g,"").replace(/縣長 {0,5}張花冠/g,"張花冠").replace(/\./g, "、").split('、'),
	abstract :  $('#ctl00_ContentPlaceHolder1_fvDetail_lbCaseName').text(),
	description : $('#ctl00_ContentPlaceHolder1_fvDetail_lbContent').text(),
	method : $('#ctl00_ContentPlaceHolder1_fvDetail_lbWay').text(),
	motions : [
		{
            date : null, 
            motion : "審查意見", 
            resolution : $('#ctl00_ContentPlaceHolder1_fvDetail_lbOpinion').text()
        }, 
        {
            date : null, 
            motion : "大會決議", 
            resolution : $('#ctl00_ContentPlaceHolder1_fvDetail_lbResolution').text()
        }
        ]
